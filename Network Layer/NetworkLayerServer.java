...
public class NetworkLayerServer {
    ...
    public static synchronized void DVR(int startingRouterId) {
        ...
        //System.out.println("Starting DVR");
        ...
        while (x)
        {
            ...
            for(int nid : r.getNeighborRouterIDs())
            {
                Router neighbour=routerMap.get(nid);
                if(neighbour.getState())
                {
                    boolean y=neighbour.sfupdateRoutingTable(r);
                    x = y ? y : x;
                }
            }

            for(Map.Entry<Integer, Router> routerEntry:routerMap.entrySet())
            {
                Integer i=routerEntry.getKey();
                if(i!=startingRouterId){

                    Router r1=routerEntry.getValue();

                    for(int nid : r1.getNeighborRouterIDs())
                    {
                        Router neighbour=routerMap.get(nid);
                        if(neighbour.getState())
                        {
                            boolean y=neighbour.sfupdateRoutingTable(r1);
                            x = y ? y : x;
                        }
                    }
                }
        ...
        //System.out.println("Done with DVR");
    }
    ...
    public static synchronized void simpleDVR(int startingRouterId) {
        ...
        while (x)
        {
            ...
            for(int nid : r.getNeighborRouterIDs())
            {
                ...
                if(neighbour.getState())
                {
                    boolean y=neighbour.updateRoutingTable(r);
                    x = y ? y : x;
                }
            }
            ...
            for(Map.Entry<Integer, Router> routerEntry:routerMap.entrySet()) {
                ...
                if (i != startingRouterId) {
                    ...
                    for (int nid : r1.getNeighborRouterIDs()) {
                        Router neighbour = routerMap.get(nid);
                        if (neighbour.getState()) {
                            boolean y = neighbour.updateRoutingTable(r1);
                            x = y ? y : x;
                        }
                    }
                }
